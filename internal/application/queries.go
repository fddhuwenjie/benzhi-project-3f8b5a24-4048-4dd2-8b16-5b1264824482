...
func (s *Service) EvidenceCredential(id string, query EvidenceQuery) (EvidenceCredential, error) {
	...
	selection, cached := s.cachedEvidenceSelection(id)
	var err error
	if !cached {
		...
		if err == nil {
			s.cacheEvidenceSelection(id, selection)
		}
	}
...
func (s *Service) cachedEvidenceSelection(id string) (audit.EvidenceSelection, bool) {
	s.mu.Lock()
	defer s.mu.Unlock()
	selection, ok := s.evidenceSelections[id]
	selection.Items = append([]audit.EvidenceItem(nil), selection.Items...)
	return selection, ok
}

func (s *Service) cacheEvidenceSelection(id string, selection audit.EvidenceSelection) {
	s.mu.Lock()
	defer s.mu.Unlock()
	selection.Items = append([]audit.EvidenceItem(nil), selection.Items...)
	s.evidenceSelections[id] = selection
}
